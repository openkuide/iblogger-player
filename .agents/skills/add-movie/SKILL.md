...
- **Poster** — For official dramas, if no URL is available, use `"./assets/images/<slug>.png"` and remind the user to add it. For Douyin Chinese short movies, always generate a high-quality cinematic PNG poster using the `generate_image` tool, save it locally to `assets/images/<slug>.png`, and reference it in the metadata. The prompt should specify a cinematic ancient/modern Chinese drama poster style (Theatrical Calligraphy style) featuring:
  - Centered at the top: the movie's title in large, elegant Chinese calligraphic brush strokes (Chinese characters).
  - Directly underneath the Chinese title: the English title in a smaller, clean sans-serif/classic Roman font.
  - At the bottom overlay: small clean credit text listing lead actors (e.g. 'Starring: [Actor Names]'), director names, a release tag 'IN THEATERS THIS YEAR' or release year, and small production studio logo icons in the bottom corners to make it look like an authentic official theatrical release poster.
  - Dramatic lighting, authentic character actions, detailed clothing, and settings matching the genre.
