...
description: >
  ...
  automatically after the Diamond Iterative Process loop finds a lesson learned
...
## Purpose
This skill makes Antigravity a **self-improving agent**. After completing any
...
1. **Redundancy**: Are the same rules or guidelines repeated in multiple
   configuration files (`AGENTS.md`, `.cursorrules`, `CLAUDE.md`,
   `copilot-instructions.md`)? Flag duplicates.
...
**Output**: A bulleted list of findings for each configuration file, with file
path and line number where possible.
...
| Skill instructions | `.agents/skills/<skill>/SKILL.md` |
...
4. **Recommend Compaction**: Suggest the user run `/compact` (Claude Code) to
   free token budget before continuing to the next task.
...
- The conversation is approaching **80% of the context window** (token budget
  warning).
...
- [ ] Compacted session summary written to walkthrough artifact
...
- `.agents/AGENTS.md` — Primary rule set for this workspace
- `CLAUDE.md` — Claude Code rule set
